...
void otaThread::setCmd(enum canComStep mode)
{
    ...
    case STEP_SET_MOTOR_MODE:
        ...
        else if(motor_mode_set == MOTOR_ENCDOER_CALI_9)
        {
            pack.exId.mode =  CANCOM_MOTOR_ENCODER;
...
//模式分发
            ...
            case CANCOM_MOTOR_SWEEP:
                rxdPack_MOTOR_SWEEP();
                break;
            case  CANCOM_MOTOR_ENCODER:
                rxdPack_MOTOR_ENCDOER();
                break;
...
void otaThread::rxdPack_MOTOR_FEEDBACK()
{
    ...
    torqueGet = torqueIntGet / 10;//uint_to_float(torqueIntGet, T_MIN, T_MAX, 16);
...
void otaThread::rxdPack_MOTOR_ENCDOER()
{
        memcpy(&currentAngle, &rxFrame.data[0], 4);
        memcpy(&rotorAngle, &rxFrame.data[4], 4);
        // 发出信号通知界面更新
        otaStatus = OTA_STA_ENCODER_CALI;
        emit sendState();
}
        ...
        case 0:
            if (chSelPos >= chSelNum)
            {
                ...
                if (scopeShowNum == 0)
                    scopeShowNum = 1;
                scopeShowCount = 0;
                scopeWindowNum = (quint32)(WAVE_TIME_WINDOW * chEchoFreHz)*5;
                scopeWindowCount = 0;
...
void otaThread::scope_dataProcess()
{
    quint16 tick;
      ...
      // 统一时间计算方式，确保Debug和Release模式下一致
      if (Burst_mode) {
          // 突发模式：假设tick单位为微秒
          if (tick > chTickBak) {
              chTimeStamp += (tick - chTickBak) / 1000000.0f;
          } else {
              chTimeStamp += ((65536 + tick - chTickBak) / 1000000000.0f)/2.5;
          }
          chTickBak = tick;
      } else {
          // 非突发模式：使用chTickUs作为时间单位（假设为微秒）
          if (tick > chTickBak) {
              chTimeStamp +=(( (tick - chTickBak) * chTickUs) / 1000000000.0f)/2.5;
          } else {
              chTimeStamp += ((65536 + tick - chTickBak) * chTickUs / 1000000000.0f)/2.5;
          }
          chTickBak = tick;
      }
