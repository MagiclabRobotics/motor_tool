...
extern int first_through;
// 设置控制模式的选项及名称
QVector<_MODE_NAME> ctrl_mode_name =
    {
        {MOTOR_RESET, "200 复位"},
        {MOTOR_PARALL_CLOSE_LOOP_LOWSIDE, "1 PARALL低编"},
        {MOTOR_CASCADE_CLOSE_LOOP_LOWSIDE, "2 CASCADE低编"},
        {MOTOR_PARALL_CLOSE_LOOP_HIGHSIDE, "3 PARALL高编"},
        {MOTOR_CASCADE_CLOSE_LOOP_HIGHSIDE, "4 CASCADE高编"},
        {MOTOR_ADRC_CLOSE_LOOP, "5 ADRC"},
        {MOTOR_TORQUE_CTRL_LOWSIDE, "6 TORQUE"},
        {MOTOR_VELOCITY_CTRL_LOWSIDE, "7 VELOCITY"},
        {MOTOR_CURLOOP_FRQ_SWEEP, "17 扫频"},
        {MOTOR_TORQ_CALI_TimeMode, "248 扭矩步进"},
        {MOTOR_OPEN_LOOP, "253 开环转"},
        {MOTOR_RESET, "自定义模式"}, // 自定义模式不需要数字，这里只用作占位
};
    ...
    : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    ui->lineEditCustomPara0->setValidator(new QIntValidator(0, 255, this));
    ui->lineEditCustomPara1->setValidator(new QDoubleValidator());
    ui->lineEditCustomPara2->setValidator(new QDoubleValidator());
    ui->lineEditCustomPara3->setValidator(new QDoubleValidator());
    ui->lineEditCustomPara4->setValidator(new QDoubleValidator());
    ui->lineEditCustomPara5->setValidator(new QDoubleValidator());
    ui->lineEditCustomPara6->setValidator(new QDoubleValidator());
    ui->lineEditCustomPara7->setValidator(new QDoubleValidator());
    ui->lineEditCustomPara8->setValidator(new QDoubleValidator());
    ui->lineEditCustomPara0->setText("0");
    ui->lineEditCustomPara1->setText("0");
    ui->lineEditCustomPara2->setText("0");
    ui->lineEditCustomPara3->setText("0");
    ui->lineEditCustomPara4->setText("0");
    ui->lineEditCustomPara5->setText("0");
    ui->lineEditCustomPara6->setText("0");
    ui->lineEditCustomPara7->setText("0");
    ui->lineEditCustomPara8->setText("0");

    ui->lineEditSetPos_Tar->setValidator(new QDoubleValidator());
    ui->lineEditSetPos_Kp->setValidator(new QDoubleValidator());
    ui->lineEditSetPos_Ki->setValidator(new QDoubleValidator());
    ui->lineEditSetVel_Tar->setValidator(new QDoubleValidator());
    ui->lineEditSetVel_Kp->setValidator(new QDoubleValidator());
    ui->lineEditSetVel_Ki->setValidator(new QDoubleValidator());
    ui->lineEditSetTor_Tar->setValidator(new QDoubleValidator());
    ui->lineEditSetTor_Kp->setValidator(new QDoubleValidator());
    // ui->lineEditSetTor_Ki->setValidator(new QDoubleValidator());
    ui->lineEditSetPos_Tar->setText("0");
    ui->lineEditSetPos_Kp->setText("0");
    ui->lineEditSetPos_Ki->setText("0");
    ui->lineEditSetVel_Tar->setText("0");
    ui->lineEditSetVel_Kp->setText("0");
    ui->lineEditSetVel_Ki->setText("0");
    ui->lineEditSetTor_Tar->setText("0");
    ui->lineEditSetTor_Kp->setText("0");
    // ui->lineEditSetTor_Ki->setText("0");
    ui->lineEdit_Err_Code->setText("");
    ui->lineEdit_Cur_Stat->setText("");
    ui->lineEditSetPos_Tar->setEnabled(false);
    ui->lineEditSetPos_Kp->setEnabled(false);
    ui->lineEditSetPos_Ki->setEnabled(false);
    ui->lineEditSetVel_Tar->setEnabled(false);
    ui->lineEditSetVel_Kp->setEnabled(false);
    ui->lineEditSetVel_Ki->setEnabled(false);
    ui->lineEditSetTor_Tar->setEnabled(false);
    ui->lineEditSetTor_Kp->setEnabled(false);
    // ui->lineEditSetTor_Ki->setEnabled(false);

    ui->lineEditSetTor_Kp->setVisible(false);
    // ui->lineEditSetTor_Ki->setVisible(false);
    // ui->label_33->setVisible(false);
    ui->label_25->setVisible(false);

    ui->label_mode_self->setVisible(false);
    ui->lineEdit_Mode_Self->setVisible(false);
    ui->lineEdit_Mode_Self->setText("0");
    ui->lineEdit_Mode_Self->setValidator(new QIntValidator(0, 255, this));
    ...
    ui->waveShow->setOpenGl(true);
#endif

    // 设置下拉框的选项
    for (int i = 0; i < ctrl_mode_name.length(); i++)
    {
        ui->comboBox_CtrlMode->addItem(ctrl_mode_name[i].m_name);
    }
    ui->comboBox_CtrlMode->setEnabled(false);

    ui->lineEditSetEchoFre->setText("100");
    uart2can.chEchoFreHz = 100;

    uart2can.chEchoFreHzMax = SCOPE_FRE_MAX;
    ...
    connect(timer0, SIGNAL(timeout()), this, SLOT(t0_process()));
    timer0->start(1);
...
void MainWindow::noCanDevConfig()
{
    ...
    ui->pushButtonSavePara->setEnabled(false);
    ui->lineEditCustomPara0->setEnabled(false);
    ui->lineEditCustomPara1->setEnabled(false);
    ui->lineEditCustomPara2->setEnabled(false);
    ui->lineEditCustomPara3->setEnabled(false);
    ui->lineEditCustomPara4->setEnabled(false);
    ui->lineEditCustomPara5->setEnabled(false);
    ui->lineEditCustomPara6->setEnabled(false);
    ui->lineEditCustomPara7->setEnabled(false);
    ui->lineEditCustomPara8->setEnabled(false);
    ui->pushButtonCustomeCMD->setEnabled(false);

    ui->comboBox_CtrlMode->setEnabled(false);
    ui->comboBox_CtrlMode->setCurrentIndex(0);
    ui->lineEditSetPos_Tar->setEnabled(false);
    ui->lineEditSetPos_Kp->setEnabled(false);
    ui->lineEditSetPos_Ki->setEnabled(false);
    ui->lineEditSetVel_Tar->setEnabled(false);
...
void MainWindow::detectedCanDevConfig()
{
    ...
    ui->pushButtonSavePara->setEnabled(true);
    ui->lineEditCustomPara0->setEnabled(true);
    ui->lineEditCustomPara1->setEnabled(true);
    ui->lineEditCustomPara2->setEnabled(true);
    ui->lineEditCustomPara3->setEnabled(true);
    ui->lineEditCustomPara4->setEnabled(true);
    ui->lineEditCustomPara5->setEnabled(true);
    ui->lineEditCustomPara6->setEnabled(true);
    ui->lineEditCustomPara7->setEnabled(true);
    ui->lineEditCustomPara8->setEnabled(true);
    ui->pushButtonCustomeCMD->setEnabled(true);

    ui->pushButtonSavePara->setEnabled(true);
    ui->comboBox_CtrlMode->setEnabled(true);
}
...
void MainWindow::on_pushButtonOpenCom_clicked()
{
    ...
    if (false == IsComOpen)
    {
        ...
        {
            // 以下是串口设置。
            myCom->setBaudRate(COM_BOUND_DEFAULT);
            ...
            ui->statusBar->showMessage("成功打开串口：" + QString::number(COM_BOUND_DEFAULT) + ",8,1,0"); // 这个跟显示的可能不太一样，可能是版本的问题
...
void MainWindow::StateUpdate()
{
    ...
    case OTA_STA_DETECT_MCU:
        ...
        if (uart2can.mcuBuf.changeFlag == true)
        {
            ...
            {
                str = "MOT:" + QString::number(uart2can.mcuBuf.canId[i], 10) + "  uid:" + QString::number(uart2can.mcuBuf.id[i], 16);
    ...
    case OTA_STA_ENCODER_CALI_RESULT:
        ui->textEditCfgInfo->append("编码器标定结果-设备：" + QString::number(uart2can.mcuCanId, 10) + " 编号：" + QString::number(uart2can.caliI, 10) + " 数组序号：" + QString::number(uart2can.caliInd, 10) + " 校正值：" + QString::number(uart2can.caliLut, 'f', 8));
        break;
    case OTA_STA_ENCODER_CALI_DONE:
    {
        QString str_out = "编码器标定完成-设备：" + QString::number(uart2can.mcuCanId, 10) //
        + " Encoder Electrical Offset：" + QString::number(uart2can.caliOffset, 'f', 6) //
        + " 电角度均方差：" + QString::number(uart2can.variance, 'f', 6) ;//
        if((uart2can.calerrorcode&ERROR_CALI)==ERROR_CALI)
        {
            str_out += " 标定异常！！！";
        }
        ui->textEditCfgInfo->append(str_out);
        // ui->textEditCfgInfo->append(str);
        break;
    }
    case OTA_STA_HALL_CALI_ING:
        ui->textEditCfgInfo->append("Hall标定中-设备：" + //
        QString::number(uart2can.mcuCanId, 10) + " 序号：" + //
        QString::number(uart2can.hallCount, 10) + " hall值 1：" + //
        QString::number(uart2can.hallRaw[0], 10) + " 2：" + //
        QString::number(uart2can.hallRaw[1], 10) + " 3：" + //
        QString::number(uart2can.hallRaw[2], 10) + " 4：" + //
        QString::number(uart2can.hallRaw[3], 10) + " 5：" + //
        QString::number(uart2can.hallRaw[4], 10) + " 6：" + //
        QString::number(uart2can.hallRaw[5], 10));
        break;
    case OTA_STA_HALL_CALI_RESULT1:
        ui->textEditCfgInfo->append("Hall标定结果1-设备：" + QString::number(uart2can.mcuCanId, 10) + " 序号：" + QString::number(uart2can.hallCount, 10) + " 查表值(右-左)：" + QString::number(uart2can.hallRdLval, 10));
        break;
    case OTA_STA_HALL_CALI_RESULT2:
        ui->textEditCfgInfo->append("Hall标定结果2-设备：" + QString::number(uart2can.mcuCanId, 10) + //
        " HALL编号：" //+QString::number(uart2can.hallCount,10)
        + //
    ...
    case OTA_STA_HALL_CALI_DONE:
        str = "HALL标定完成-设备：" + QString::number(uart2can.mcuCanId, 10) + " 故障：" //+QString::number(uart2can.hallError,16)
    ...
    case OTA_STA_WRITE_PARA_SUCCESS:
        ...
        if (writeAllModyfiedFlag)
        {
            ...
            {
                // uart2can.setCmd(STEP_SAVE_PARA_STR); // 写成功后先不主动保存
                ui->labelParaInfo->setStyleSheet("color:green;");
                ui->labelParaInfo->setText("所有参数写成功！");
            }
        }
        break;
    ...
    case OTA_STA_PARA_UPDATE_SUCCESS_END:
        ...
        break;
    case OTA_STA_GET_CUSTOME_STR:
        str = uart2can.custome_str;
        font_1.setFamily("Microsoft YaHei"); // 设置字体家族
        // font_1.setPointSize(12);             // 设置字体大小
        // font_1.setBold(false);               // 明确设置不加粗
        if(set_format_style){
            set_format_style = 0;
            // boldFormat.setFontWeight(QFont::Bold);
            // ui->textEditCfgInfo->setCurrentCharFormat(boldFormat);
            font_1.setBold(true);

            ui->textEditCfgInfo->setCurrentFont(font_1);
        }
        else{
        ...
        ui->textEditCfgInfo->append(str);
        switch(uart2can.custome_strPlace)
        {
        case STR_PALCE_PARA_OK:
            ui->labelParaInfo->setStyleSheet("color:green;");
            ui->labelParaInfo->setText(uart2can.custome_str);
        break;
        case STR_PALCE_PARA_ERR:
            ui->labelParaInfo->setStyleSheet("color:red;");
            ui->labelParaInfo->setText(uart2can.custome_str);
        break;
        default:
        ...
        break;
    case OTA_STA_SCOPE_CONTINUE_CONFLICT:
        ui->label_selChInfo->setStyleSheet("color:red;");
        ui->label_selChInfo->setText("示波器模式与连续发送冲突！");
        ui->labelCtrlInfo->setStyleSheet("color:red;");
        ui->labelCtrlInfo->setText("示波器模式与连续发送冲突！");
        break;
    default:
        break;
    }
...
void MainWindow::on_pushButtonCfgContinous_clicked()
{
    ...
    if(uart2can.ScopeMode == 1)
    {
        uart2can.otaStatus = OTA_STA_SCOPE_CONTINUE_CONFLICT;
        StateUpdate();
        return;
    }
    int pos = ui->comboBoxDev->currentIndex();
...
void MainWindow::on_pushButtonCfgMotor_clicked() // 对应运行按钮
{
    ...
    {
        bool ok;
        int mode = ui->comboBox_CtrlMode->currentIndex();
        if(ctrl_mode_name[mode].m_num == MOTOR_CURLOOP_FRQ_SWEEP){
            first_through = 0;
        }

        if (mode == (ctrl_mode_name.size() - 1))
        { // 读取自定义模式
            int t = ui->lineEdit_Mode_Self->text().toInt(&ok);
            if ((ok == false) || (t > 255) || (t < 0))
            {
                ui->textEditCfgInfo->setStyleSheet("color:red;");
                ui->textEditCfgInfo->setText("自定义模式超范围！");
                return;
            }
            else
            {
                uart2can.MotCtrlMode = t;
            }
        }
        else
        {
            uart2can.MotCtrlMode = ctrl_mode_name[mode].m_num;
        }
    }
...
void MainWindow::on_pushButtonCustomeCMD_clicked()
{
    int pos = ui->comboBoxDev->currentIndex();
    uart2can.mcuBuf.usePos = (quint8)pos;
    uart2can.devCanId = uart2can.mcuBuf.canId[uart2can.mcuBuf.usePos];

    uart2can.cust_cmd = ui->lineEditCustomPara0->text().toInt();
    uart2can.cust_cmd_para[0] = ui->lineEditCustomPara1->text().toFloat();
    uart2can.cust_cmd_para[1] = ui->lineEditCustomPara2->text().toFloat();
    uart2can.cust_cmd_para[2] = ui->lineEditCustomPara3->text().toFloat();
    uart2can.cust_cmd_para[3] = ui->lineEditCustomPara4->text().toFloat();
    uart2can.cust_cmd_para[4] = ui->lineEditCustomPara5->text().toFloat();
    uart2can.cust_cmd_para[5] = ui->lineEditCustomPara6->text().toFloat();
    uart2can.cust_cmd_para[6] = ui->lineEditCustomPara7->text().toFloat();
    uart2can.cust_cmd_para[7] = ui->lineEditCustomPara8->text().toFloat();
    uart2can.unAckedCnt = 0;
    uart2can.setCmd(STEP_SET_CUSTOME_CMD);
}
...
// 保存参数
void MainWindow::on_pushButtonSavePara_clicked()
{
    int pos = ui->comboBoxDev->currentIndex();
    uart2can.mcuBuf.usePos = (quint8)pos;
    uart2can.devCanId = uart2can.mcuBuf.canId[uart2can.mcuBuf.usePos];
...
void MainWindow::on_pushButtonEchoSet_clicked()
{
    ...
    quint32 needFre = uart2can.chEchoFreHz;
...
void MainWindow::on_pushButtonEchoStart_clicked()
{
    ...
    if (str == "启动")
    {
        ...
        // {
        //     ui->label_selChInfo->setStyleSheet("color:red;");
        //     ui->label_selChInfo->setText("示波器已经开启，请先停止！");
        //     return;
        // }
        if(uart2can.ContinueMode == 1)
        {
            uart2can.otaStatus = OTA_STA_SCOPE_CONTINUE_CONFLICT;
            StateUpdate();
            return;
...
void MainWindow::t0_process()
{
    ...
    {
        t0s = 0;
        // qDebug()<<"tt" << myCom->isOpen() << uart2can.devCanId;
    }
    // if(myCom->isOpen())
    // {
    //     readFromCom();
...
void MainWindow::on_comboBox_CtrlMode_currentIndexChanged(int index)
{
    int mode = ui->comboBox_CtrlMode->currentIndex();
    if (mode >= ctrl_mode_name.size())
    {

        ui->labelParaInfo->setStyleSheet("color:red;");
        ui->labelParaInfo->setText("模式选项越界！");
        return;
    }
    ...
    ui->lineEditSetTor_Kp->setEnabled(false);
    // ui->lineEditSetTor_Ki->setEnabled(false);
    ui->lineEdit_Err_Code->setEnabled(false);
    ui->lineEdit_Cur_Stat->setEnabled(false);

    if (mode == (ctrl_mode_name.size() - 1))
    {
        ui->label_mode_self->setVisible(true);
        ui->lineEdit_Mode_Self->setVisible(true);
        ui->lineEditSetPos_Tar->setEnabled(true);
        ui->lineEditSetPos_Kp->setEnabled(true);
        ui->lineEditSetPos_Ki->setEnabled(true);
        ui->lineEditSetVel_Tar->setEnabled(true);
        ui->lineEditSetVel_Kp->setEnabled(true);
        ui->lineEditSetVel_Ki->setEnabled(true);
        ui->lineEditSetTor_Tar->setEnabled(true);
        // ui->lineEditSetTor_Kp->setEnabled(true);
        // ui->lineEditSetTor_Ki->setEnabled(true);
    }
    else
    {
        ui->label_mode_self->setVisible(false);
        ...
        MOTOR_DRV_STATE ctrl_mode = (MOTOR_DRV_STATE)ctrl_mode_name[mode].m_num;
        switch (ctrl_mode)
        {
        case MOTOR_RESET:
            break;
        case MOTOR_PARALL_CLOSE_LOOP_LOWSIDE:
            ui->lineEditSetPos_Tar->setEnabled(true);
            ui->lineEditSetPos_Kp->setEnabled(true);
            ui->lineEditSetPos_Ki->setEnabled(true);
            ui->lineEditSetVel_Tar->setEnabled(true);
            ...
            // ui->lineEditSetTor_Ki->setEnabled(true);
            break;
        case MOTOR_CASCADE_CLOSE_LOOP_LOWSIDE:
            ui->lineEditSetPos_Tar->setEnabled(true);
            ui->lineEditSetPos_Kp->setEnabled(true);
            ui->lineEditSetPos_Ki->setEnabled(true);
            ui->lineEditSetVel_Tar->setEnabled(true);
            ...
            // ui->lineEditSetTor_Ki->setEnabled(true);
            break;
        case MOTOR_PARALL_CLOSE_LOOP_HIGHSIDE:
            ui->lineEditSetPos_Tar->setEnabled(true);
            ui->lineEditSetPos_Kp->setEnabled(true);
            ui->lineEditSetPos_Ki->setEnabled(true);
            ui->lineEditSetVel_Tar->setEnabled(true);
            ...
            // ui->lineEditSetTor_Ki->setEnabled(true);
            break;
        case MOTOR_CASCADE_CLOSE_LOOP_HIGHSIDE:
            ui->lineEditSetPos_Tar->setEnabled(true);
            ui->lineEditSetPos_Kp->setEnabled(true);
            ui->lineEditSetPos_Ki->setEnabled(true);
            ui->lineEditSetVel_Tar->setEnabled(true);
        ...
        case MOTOR_ADRC_CLOSE_LOOP:
            ...
            // ui->lineEditSetTor_Ki->setEnabled(true);
            break;
        case MOTOR_TORQUE_CTRL_LOWSIDE:
            ui->lineEditSetTor_Tar->setEnabled(true);
            // ui->lineEditSetTor_Kp->setEnabled(true);
            // ui->lineEditSetTor_Ki->setEnabled(true);
            break;
        case MOTOR_VELOCITY_CTRL_LOWSIDE:
            ui->lineEditSetVel_Tar->setEnabled(true);
            ui->lineEditSetVel_Kp->setEnabled(true);
            ui->lineEditSetVel_Ki->setEnabled(true);
            ui->lineEditSetTor_Tar->setEnabled(true);
            ...
            // ui->lineEditSetTor_Ki->setEnabled(true);
            break;
        case MOTOR_TORQ_CALI_TimeMode:
            ui->lineEditSetTor_Tar->setEnabled(true);
            break;
        case MOTOR_CURLOOP_FRQ_SWEEP:
            break;
...
void MainWindow::on_lineEditSetPos_Tar_editingFinished()
{
    bool ok;
    float val = ui->lineEditSetPos_Tar->text().toFloat(&ok);
    if (ok == true)
    {
        uart2can.pos_tarSet = val;
    }
    else
    {
        uart2can.pos_tarSet = 0;
        ui->lineEditSetPos_Tar->setText("0");
    }
    ui->labelCtrlInfo->setStyleSheet("color:green;");
    ui->labelCtrlInfo->setText("角度设置： " + QString::number(uart2can.pos_tarSet, 'f', 4) + " rad");
}

void MainWindow::on_lineEditSetPos_Kp_editingFinished()
{
    bool ok;
    float val = ui->lineEditSetPos_Kp->text().toFloat(&ok);
    if (ok == true)
    {
        uart2can.pos_kpSet = val;
    }
    else
    {
        uart2can.pos_kpSet = 0;
        ui->lineEditSetPos_Kp->setText("0");
    }
    ui->labelCtrlInfo->setStyleSheet("color:green;");
    ui->labelCtrlInfo->setText("角度比例系数： " + QString::number(uart2can.pos_kpSet, 'f', 4));
}

void MainWindow::on_lineEditSetPos_Ki_editingFinished()
{
    bool ok;
    float val = ui->lineEditSetPos_Ki->text().toFloat(&ok);
    if (ok == true)
    {
        uart2can.pos_kiSet = val;
    }
    else
    {
        uart2can.pos_kiSet = 0;
        ui->lineEditSetPos_Ki->setText("0");
    }
    ui->labelCtrlInfo->setStyleSheet("color:green;");
    ui->labelCtrlInfo->setText("角度积分系数： " + QString::number(uart2can.pos_kiSet, 'f', 4));
}

void MainWindow::on_lineEditSetVel_Tar_editingFinished()
{
    bool ok;
    float val = ui->lineEditSetVel_Tar->text().toFloat(&ok);
    if (ok == true)
    {
        uart2can.vel_tarSet = val;
    }
    else
    {
        uart2can.vel_tarSet = 0;
        ui->lineEditSetVel_Tar->setText("0");
    }
    ui->labelCtrlInfo->setStyleSheet("color:green;");
    ui->labelCtrlInfo->setText("速度设置： " + QString::number(uart2can.vel_tarSet, 'f', 4) + " rad/s");
}

void MainWindow::on_lineEditSetVel_Kp_editingFinished()
{
    bool ok;
    float val = ui->lineEditSetVel_Kp->text().toFloat(&ok);
    if (ok == true)
    {
        uart2can.vel_kpSet = val;
    }
    else
    {
        uart2can.vel_kpSet = 0;
        ui->lineEditSetVel_Kp->setText("0");
    }
    ui->labelCtrlInfo->setStyleSheet("color:green;");
    ui->labelCtrlInfo->setText("速度比例系数： " + QString::number(uart2can.vel_kpSet, 'f', 4));
}

void MainWindow::on_lineEditSetVel_Ki_editingFinished()
{
    bool ok;
    float val = ui->lineEditSetVel_Ki->text().toFloat(&ok);
    if (ok == true)
    {
        uart2can.vel_kiSet = val;
    }
    else
    {
        uart2can.vel_kiSet = 0;
        ui->lineEditSetVel_Ki->setText("0");
    }
    ui->labelCtrlInfo->setStyleSheet("color:green;");
    ui->labelCtrlInfo->setText("速度积分系数： " + QString::number(uart2can.vel_kiSet, 'f', 4));
}

void MainWindow::on_lineEditSetTor_Tar_editingFinished()
{
    bool ok;
    float val = ui->lineEditSetTor_Tar->text().toFloat(&ok);
    if (ok == true)
    {
        uart2can.tor_tarSet = val;
    }
    else
    {
        uart2can.tor_tarSet = 0;
        ui->lineEditSetTor_Tar->setText("0");
    }
    ui->labelCtrlInfo->setStyleSheet("color:green;");
    ui->labelCtrlInfo->setText("力矩设置： " + QString::number(uart2can.tor_tarSet, 'f', 4) + " Nm");
}

void MainWindow::on_lineEditSetTor_Kp_editingFinished()
{
    bool ok;
    float val = ui->lineEditSetTor_Kp->text().toFloat(&ok);
    if (ok == true)
    {
        uart2can.tor_kpSet = val;
    }
    else
    {
        uart2can.tor_kpSet = 0;
        ui->lineEditSetTor_Kp->setText("0");
    }
    ui->labelCtrlInfo->setStyleSheet("color:green;");
    ui->labelCtrlInfo->setText("力矩比例系数： " + QString::number(uart2can.tor_kpSet, 'f', 4));
}

// void MainWindow::on_lineEditSetTor_Ki_editingFinished()
// {
//     bool ok;
//     float val = ui->lineEditSetTor_Ki->text().toFloat(&ok);
//     if (ok == true)
//     {
//         uart2can.tor_kiSet = val;
//     }
//     else
//     {
//         uart2can.tor_kiSet = 0;
//         ui->lineEditSetTor_Ki->setText("0");
//     }
//     ui->labelCtrlInfo->setStyleSheet("color:green;");
//     ui->labelCtrlInfo->setText("力矩积分系数： " + QString::number(uart2can.tor_kiSet, 'f', 4));
// }

void MainWindow::on_lineEdit_Mode_Self_editingFinished()
{
    bool ok;
    int val = ui->lineEdit_Mode_Self->text().toInt(&ok);
    if (ok == true)
    {
        if(val>255)
        {
            ui->lineEdit_Mode_Self->setText("255");
        }
        if(val<0)
        {
            ui->lineEdit_Mode_Self->setText("0");
...
void MainWindow::on_lineEditCustomPara0_editingFinished()
{
    bool ok;
    int val = ui->lineEditCustomPara0->text().toInt(&ok);
    if (ok == true)
    {
        if(val>255)
        {
            val = 255;
            ui->lineEditCustomPara0->setText("255");
        }
        if(val<0)
        {
            val = 0;
