...
void MainWindow::StateUpdate()
{
    ...
    case OTA_STA_ENCODER_CALI_ING:
        ...
        break;
        ui->textEditCfgInfo->setFontFamily("Courier New");
    ...
    case OTA_STA_MOTOR_FEEDBACK:
        ...
        str = "电机：" + QString::number(uart2can.canIdGet, 10) + " 位置：" + QString::number(uart2can.posGet, 'f', 4) + " 速度：" + QString::number(uart2can.velGet, 'f', 4) + " 力矩：" + QString::number(uart2can.torqueGet, 'f', 4)
            ...
            ((uart2can.mtStatus.mode == MT_MODE_MOTOR_248) ? "248_扭矩步进" : ((uart2can.mtStatus.mode == MT_MODE_MOTOR_253) ? "253_开环转" :  ((uart2can.mtStatus.mode == MT_MODE_MOTOR_17) ? "17_扫频":  ((uart2can.mtStatus.mode ==MT_MODE_MOTOR_9) ? "9_高编自校准":((uart2can.mtStatus.mode == MT_MODE_BRAKE) ? "BRAKE" : "RESET"))))))))
...
void MainWindow::on_combox_motor_ID_currentIndexChanged(int index)
{
    ...
    else if (ui->combox_motor_ID->currentText() == "防水电机")
    {
        P_MIN = -12.5f;
        P_MAX = 12.5f;
        V_MIN = -45.0f;
...
void MainWindow::on_comboBox_mot_mode_currentIndexChanged(int index)
{
    ...
    else if (ui->comboBox_mot_mode->currentText() == "9 高编自校准")
    {
        motor_mode_set = MOTOR_ENCDOER_CALI_9 ;
    }
