    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(&this->uart2can, SIGNAL(WriteToSerial(QByteArray,qint16)), this, SLOT(writeToCom(QByteArray,qint16)));
...
//    ui->waveShow->setNotAntialiasedElement(QCP::aeAll);
//    ui->waveShow->setNoAntialiasingOnDrag(true);
//    ui->waveShow->setPlottingHints(QCP::phFastPolylines);
...
QString MainWindow::getParaTypeStr(enum paraType type)
{
    ...
    switch (type) {
    case ParaType_U8     :  str = "uint8"; break;
    case ParaType_S8     :  str = "int8"; break;
    case ParaType_U16    :  str = "uint16"; break;
    case ParaType_S16    :  str = "int16"; break;
    case ParaType_U32    :  str = "uint32"; break;
    case ParaType_S32    :  str = "int32"; break;
    case ParaType_FLOAT  :  str = "float"; break;
    case ParaType_U64    :  str = "uint64"; break;
    case ParaType_S64    :  str = "int64"; break;
    case ParaType_DOUBLE :  str = "double"; break;
...
QString MainWindow::getParaValStr(enum paraType type,const quint8* add)
{
    ...
    switch (type) {
    case ParaType_U8     :quint8  tempU8 ;memcpy(&tempU8 ,add,1);str=QString::number(tempU8 , 10);break;
    case ParaType_S8     : qint8  tempS8 ;memcpy(&tempS8 ,add,1);str=QString::number(tempS8 , 10);break;
    case ParaType_U16    :quint16 tempU16;memcpy(&tempU16,add,2);str=QString::number(tempU16, 10);break;
    case ParaType_S16    : qint16 tempS16;memcpy(&tempS16,add,2);str=QString::number(tempS16, 10);break;
    case ParaType_U32    :quint32 tempU32;memcpy(&tempU32,add,4);str=QString::number(tempU32, 10);break;
    case ParaType_S32    : qint32 tempS32;memcpy(&tempS32,add,4);str=QString::number(tempS32, 10);break;
    case ParaType_U64    :quint64 tempU64;memcpy(&tempU64,add,8);str=QString::number(tempU64, 10);break;
    case ParaType_S64    : qint64 tempS64;memcpy(&tempS64,add,8);str=QString::number(tempS64, 10);break;
    case ParaType_FLOAT  :  float tempF  ;memcpy(&tempF  ,add,4);str=QString::number(tempF  , 10 , 6);break;
    case ParaType_DOUBLE : double tempD  ;memcpy(&tempD  ,add,8);str=QString::number(tempD  , 10 ,12);break;
    default: str = "none";break;
    }
    return str;
}
...
QString MainWindow::setParaValStr(enum paraType type,quint8*const add,double val)
{
    ...
    switch (type) {
    case ParaType_U8     : tempU8  = val;memcpy(add,&tempU8 ,1);str=QString::number(tempU8 , 10);break;
    case ParaType_S8     : tempS8  = val;memcpy(add,&tempS8 ,1);str=QString::number(tempS8 , 10);break;
    case ParaType_U16    : tempU16 = val;memcpy(add,&tempU16,2);str=QString::number(tempU16, 10);break;
    case ParaType_S16    : tempS16 = val;memcpy(add,&tempS16,2);str=QString::number(tempS16, 10);break;
    case ParaType_U32    : tempU32 = val;memcpy(add,&tempU32,4);str=QString::number(tempU32, 10);break;
    case ParaType_S32    : tempS32 = val;memcpy(add,&tempS32,4);str=QString::number(tempS32, 10);break;
    case ParaType_U64    : tempU64 = val;memcpy(add,&tempU64,8);str=QString::number(tempU64, 10);break;
    case ParaType_S64    : tempS64 = val;memcpy(add,&tempS64,8);str=QString::number(tempS64, 10);break;
    case ParaType_FLOAT  : tempF   = val;memcpy(add,&tempF  ,4);str=QString::number(tempF  , 10 , 6);break;
    case ParaType_DOUBLE : tempD   = val;memcpy(add,&tempD  ,8);str=QString::number(tempD  , 10 ,12);break;
    default: str = "0";memset(add,0  ,8);break;
    }
    return str;
}
...
void MainWindow::paraTableClear(QTableWidget *widget)
{
    ...
    headerLabels << "功能码" << "名称"<< "参数类型"<< "属性"<< "最大值"<< "最小值"<<"当前值";
...
void MainWindow::paraTableLoadWriteStr(QTableWidget *widget,quint16 rowStart)
{
    ...
    for(i=0;i<uart2can.paraInfo.numTab[WRITE_STR];i++)
    {
        ...
        for(column=0;column<PARA_TAB_COLUMN_NUM;column++)
        {
            ...
            switch (column) {
            case 0:str = "0X"+QString("%1").arg(i|((quint16)WRITE_STR<<12),4,16,QLatin1Char('0'));
                break;
            case 1:str = uart2can.paraInfo.writeStr[i].name; break;
            case 2:str = "String"; break;
            case 3:str = "读/写"; break;
            case PARA_TAB_EDIT_COLUMN:
                pItem->setFlags(pItem->flags()|(Qt::ItemIsEditable)|(Qt::ItemIsSelectable));
                pItem->setFont( QFont( "Times", 10, QFont::Black));
                str = uart2can.paraInfo.writeStr[i].str; break;
            }
...
void MainWindow::paraTableLoadReadStr(QTableWidget *widget,quint16 rowStart)
{
    ...
    for(i=0;i<uart2can.paraInfo.numTab[READ_STR];i++)
    {
        ...
        for(column=0;column<PARA_TAB_COLUMN_NUM;column++)
        {
            ...
            switch (column) {
            case 0:str = "0X"+QString("%1").arg(i|((quint16)READ_STR<<12),4,16,QLatin1Char('0'));
                break;
            case 1:str = uart2can.paraInfo.readStr[i].name; break;
            case 2:str = "String"; break;
            case 3:str = "只读"; break;
            case PARA_TAB_EDIT_COLUMN:
                str = uart2can.paraInfo.readStr[i].str; break;
            }
...
void MainWindow::paraTableLoadWritePara(QTableWidget *widget,quint16 rowStart)
{
    ...
    for(i=0;i<uart2can.paraInfo.numTab[WRITE_PARA];i++)
    {
        ...
        for(column=0;column<PARA_TAB_COLUMN_NUM;column++)
        {
            ...
            switch (column) {
            case 0:str = "0X"+QString("%1").arg(i|((quint16)WRITE_PARA<<12),4,16,QLatin1Char('0'));
                break;
            case 1:str = uart2can.paraInfo.writePara[i].name;
                   if(i>uart2can.paraInfo.echoNum)
                        selCh_list.append(str);
                   break;
            case 2:str = getParaTypeStr(uart2can.paraInfo.writePara[i].type); break;
            case 3:str = (i>=uart2can.paraInfo.echoNum)?(uart2can.paraInfo.writePara[i].wPerm?"读/写":"设定"):"配置"; break;
            case 4:str = getParaLimitStr(uart2can.paraInfo.writePara[i].type,
                                         uart2can.paraInfo.writePara[i].max,
                                         uart2can.paraInfo.writePara[i].prec);
                break;
            case 5:str = getParaLimitStr(uart2can.paraInfo.writePara[i].type,
...
void MainWindow::paraTableLoadReadPara(QTableWidget *widget,quint16 rowStart)
{
    ...
    for(i=0;i<uart2can.paraInfo.numTab[READ_PARA];i++)
    {
        ...
        for(column=0;column<PARA_TAB_COLUMN_NUM;column++)
        {
            ...
            switch (column) {
            case 0:str = "0X"+QString("%1").arg(i|((quint16)READ_PARA<<12),4,16,QLatin1Char('0'));
                break;
            case 1:str = uart2can.paraInfo.readPara[i].name;selCh_list.append(str); break;
            case 2:str = getParaTypeStr(uart2can.paraInfo.readPara[i].type); break;
            case 3:str = "只读"; break;
            case PARA_TAB_EDIT_COLUMN:
...
void MainWindow::paraTableModifyStr(QTableWidget *widget,quint16 code)
{
    ...
    if(flag)
    {
        ...
        ui->labelParaInfo->setText("修改可写字符0X"+QString("%1").arg(code|((quint16)WRITE_STR<<12),4,16,QLatin1Char('0'))
                                   +":输入字符含有非ASCII字符!");
...
void MainWindow::paraTableModifyPara(QTableWidget *widget,quint16 code)
{
    ...
    case ParaType_U16    :
    case ParaType_U32    :
    case ParaType_U64    :  up=str.toULongLong(&ok);dp=up;break;
    case ParaType_S8     :
    case ParaType_S16    :
    case ParaType_S32    :
    case ParaType_S64    :  sp=str.toLongLong(&ok);dp=sp;break;
    case ParaType_FLOAT  :
    case ParaType_DOUBLE :  dp=str.toDouble(&ok);break;
    ...
    {
        ui->labelParaInfo->setStyleSheet("color:red;");
        ui->labelParaInfo->setText("修改可写参数0X"+QString("%1").arg(code|((quint16)WRITE_PARA<<12),4,16,QLatin1Char('0'))
                                   +":输入数据有误!");
...
void MainWindow::selConfigRefrenPara()
{
    ...
    if(uart2can.paraInfo.echoNum>0)
    {
        ...
        uart2can.chEchoFreHzMax = ((quint16)uart2can.paraInfo.writePara[uart2can.paraInfo.echoNum].max<uart2can.chEchoFreHzMax)?
                                   (quint16)uart2can.paraInfo.writePara[uart2can.paraInfo.echoNum].max:uart2can.chEchoFreHzMax;
        uart2can.chEchoFreHzMin = ((quint16)uart2can.paraInfo.writePara[uart2can.paraInfo.echoNum].min>uart2can.chEchoFreHzMin)?
                                   (quint16)uart2can.paraInfo.writePara[uart2can.paraInfo.echoNum].min:uart2can.chEchoFreHzMin;
    }
}
...
void MainWindow::paraTableRefreshModifyFlags(QTableWidget *widget,quint16 code,bool setModifiedFlag)
{
    ...
    if(setModifiedFlag == true)
    {
        ...
        switch (type) {
        case WRITE_STR:   uart2can.paraInfo.writeStr[pos].modifyFlag = true;break;
        case WRITE_PARA: uart2can.paraInfo.writePara[pos].modifyFlag = true;break;
        default:break;
        }
    ...
    else
    {
        ...
        switch (type) {
        case WRITE_STR:   uart2can.paraInfo.writeStr[pos].modifyFlag = false;break;
        case WRITE_PARA: uart2can.paraInfo.writePara[pos].modifyFlag = false;break;
        default:break;
        }
...
void MainWindow::paraTableRefreshModified(QTableWidget *widget,quint16 code)
{
    ...
    switch(type){
    case WRITE_STR : uart2can.paraInfo.writeStr[pos].str = widget->item(row,column)->text(); break;
    case WRITE_PARA: uart2can.paraInfo.writePara[pos].valStr = widget->item(row,column)->text();break;
    default:break;
    }
...
void MainWindow::paraTableRefreshVal(QTableWidget *widget,quint16 code)
{
    ...
    switch(type){
    case WRITE_STR : str = uart2can.paraInfo.writeStr[pos].str; break;
    case READ_STR  : str = uart2can.paraInfo.readStr[pos].str;  break;
    case WRITE_PARA: str = getParaValStr(uart2can.paraInfo.writePara[pos].type,
    ...
    //qDebug()<<"更新：0X"+QString::number(code,16)+" val:"+str;
...
void MainWindow::StateUpdate()
{
    ...
    case OTA_STA_ENCODER_CALI_ING    :
        ui->textEditCfgInfo->append("编码器标定中-转子角度："+QString::number(uart2can.currentAngle,'f',4)
                                   +" 定子磁链角度："+QString::number(uart2can.rotorAngle,'f',4)
                                   +" 传感器值："+QString::number(uart2can.caliRaw,10)
                                    );
    ...
    case OTA_STA_ENCODER_CALI_RESULT :
        ui->textEditCfgInfo->append("编码器标定结果-设备："+QString::number(uart2can.canIdGet,10)
                                   +" 编号："+QString::number(uart2can.caliI,10)
                                   +" 数组序号："+QString::number(uart2can.caliInd,10)
                                   +" 校正值："+QString::number(uart2can.caliLut,10)
                                    );
        break;
    case OTA_STA_ENCODER_CALI_DONE   :
        ui->textEditCfgInfo->append("编码器标定完成-设备："+QString::number(uart2can.canIdGet,10)
        ...
        str = "编码器标定完成-设备："+QString::number(uart2can.canIdGet,10)
              +" 故障："//+QString::number(uart2can.hallError,16)
              +((fault_encode)?"/异常":"")
    ...
    case OTA_STA_HALL_CALI_ING    :
        ui->textEditCfgInfo->append("Hall标定中-设备："+QString::number(uart2can.canIdGet,10)
                                    +" 序号："+QString::number(uart2can.hallCount,10)
                                    +" hall值 1："+QString::number(uart2can.hallRaw[0],10)
                                    +" 2："+QString::number(uart2can.hallRaw[1],10)
                                    +" 3："+QString::number(uart2can.hallRaw[2],10)
                                    +" 4："+QString::number(uart2can.hallRaw[3],10)
                                    +" 5："+QString::number(uart2can.hallRaw[4],10)
                                    +" 6："+QString::number(uart2can.hallRaw[5],10)
                                    );
        break;
    case OTA_STA_HALL_CALI_RESULT1:
        ui->textEditCfgInfo->append("Hall标定结果1-设备："+QString::number(uart2can.canIdGet,10)
                                    +" 序号："+QString::number(uart2can.hallCount,10)
                                    +" 查表值(右-左)："+QString::number(uart2can.hallRdLval,10)
                                    );
        break;
    case OTA_STA_HALL_CALI_RESULT2:
        ui->textEditCfgInfo->append("Hall标定结果2-设备："+QString::number(uart2can.canIdGet,10)
                                    +" HALL编号："//+QString::number(uart2can.hallCount,10)
                                    +" 最大值："+QString::number(uart2can.hallInfo[uart2can.hallCount].maxVal,10)
                                    +" 最小值："+QString::number(uart2can.hallInfo[uart2can.hallCount].minVal,10)
                                    +" 起点："+QString::number(uart2can.hallInfo[uart2can.hallCount].minPos,10)
                                    +" 中点："+QString::number(uart2can.hallInfo[uart2can.hallCount].midPos,10)
                                    +" 终点："+QString::number(uart2can.hallInfo[uart2can.hallCount].maxPos,10)
                                    +" 非单调："+QString::number(uart2can.hallInfo[uart2can.hallCount].noLinearCount,10)
                                    +" 故障："+QString::number(uart2can.hallInfo[uart2can.hallCount].fault,16)
                                    +" 警告："+QString::number(uart2can.hallInfo[uart2can.hallCount].warn,16)
                                    );
        break;
    case OTA_STA_HALL_CALI_DONE   :
        str = "HALL标定完成-设备："+QString::number(uart2can.canIdGet,10)
              +" 故障："//+QString::number(uart2can.hallError,16)
              +((uart2can.hallError&HALL_FAULT_MAX)?"/极大值":"")
              +((uart2can.hallError&HALL_FAULT_MIN)?"/极小值":"")
              +((uart2can.hallError&HALL_FAULT_POS)?"/定位点":"")
              +((uart2can.hallError&HALL_FAULT_VAL)?"/动态值":"")
              +((uart2can.hallError&HALL_FAULT_NOCALI)?"/未标定":"")
              +" 警告："+QString::number(uart2can.hallWarn,16)
              +((uart2can.hallWarn&HALL_WARN_LINEAR)?"/非单调":"")
              +((uart2can.hallWarn&HALL_WARN_DMAX)?"/查表极大值超限":"")
              +((uart2can.hallWarn&HALL_WARN_DMIN)?"/查表极小值超限":"")
              ;
        ...
        ui->textEditCfgInfo->append("HALL  编号："+QString::number(tempU8+1,10)
                                    +" 故障："//+QString::number(uart2can.hallInfo[tempU8].fault,16)
                                    +((uart2can.hallInfo[tempU8].fault&HALL_FAULT_MAX)?"/极大值":"")
                                    +((uart2can.hallInfo[tempU8].fault&HALL_FAULT_MIN)?"/极小值":"")
                                    +((uart2can.hallInfo[tempU8].fault&HALL_FAULT_POS)?"/定位点":"")
                                    +((uart2can.hallInfo[tempU8].fault&HALL_FAULT_POS)?"/定位点":"")
                                    +((uart2can.hallInfo[tempU8].fault&HALL_FAULT_VAL)?"/动态值":"")
                                    +((uart2can.hallInfo[tempU8].fault&HALL_FAULT_NOCALI)?"/未标定":"")
                                    +" 警告："//+QString::number(uart2can.hallInfo[tempU8].warn,16)
                                    +((uart2can.hallInfo[tempU8].warn&HALL_WARN_LINEAR)?"/非单调":"")
                                    +((uart2can.hallInfo[tempU8].warn&HALL_WARN_DMAX)?"/查表极大值超限":"")
                                    +((uart2can.hallInfo[tempU8].warn&HALL_WARN_DMIN)?"/查表极小值超限":"")
                                    );
        }
        break;
    ...
    case OTA_STA_MOTOR_FEEDBACK:
        ...
        str = "电机："+QString::number(uart2can.canIdGet,10)
            +" 位置："+QString::number(uart2can.posGet,'f',4)
            +" 速度："+QString::number(uart2can.velGet,'f',4)
            +" 力矩："+QString::number(uart2can.torqueGet,'f',4)
            +" 模式："
            +((uart2can.mtStatus.mode==MT_MODE_CALI)?"标定":
              ((uart2can.mtStatus.mode==MT_MODE_MOTOR)?"MOTOR":
              ((uart2can.mtStatus.mode==MT_MODE_BRAKE)?"BRAKE":"RESET")))
            +" 故障："
            +(uart2can.mtStatus.underVoltFault?"/低压":"")
            +(uart2can.mtStatus.overCurFault?"/过流":"")
            +(uart2can.mtStatus.overTempFault?"/过温":"")
            +(uart2can.mtStatus.encoderFault?"/编码器":"")
            +(uart2can.mtStatus.hallFault?"/霍尔":"")
            +(uart2can.mtStatus.noCaliFault?"/未标定":"")
                ;
...
//        break;
    ...
    case OTA_STA_GET_PARA_INFO_ERROR:
        ...
        str = "更新参数表发生错误 code:0X"+QString::number(uart2can.paraErrorCode,16)
                +((uart2can.paraErrorCode&PARA_ERROR_NO_TYPE   )?"/非法类型":"")
                +((uart2can.paraErrorCode&PARA_ERROR_TYPE      )?"/类型漏帧":"")
                +((uart2can.paraErrorCode&PARA_ERROR_POINT     )?"/编号漏帧":"")
                +((uart2can.paraErrorCode&PARA_ERROR_POINT_CONT)?"/编号不连续":"")
                +((uart2can.paraErrorCode&PARA_ERROR_NUM       )?"/超存储限制":"")
                +((uart2can.paraErrorCode&PARA_ERROR_ITEM      )?"/漏帧":"")
                +((uart2can.paraErrorCode&PARA_ERROR_ITEM_MATCH)?"/类型和数据帧不匹配":"")
                +((uart2can.paraErrorCode&PARA_ERROR_WRITE_STR_NUM_MATCH )?"/可写字符串数量错误":"")
                +((uart2can.paraErrorCode&PARA_ERROR_READ_STR_NUM_MATCH  )?"/只读字符串数量错误":"")
                +((uart2can.paraErrorCode&PARA_ERROR_WRITE_PARA_NUM_MATCH)?"/可写参数数量错误":"")
                +((uart2can.paraErrorCode&PARA_ERROR_READ_PARA_NUM_MATCH )?"/只读参数数量错误":"")
                ;
    ...
    case OTA_STA_READ_PARA_ERROR:
        ...
        str = "读取参数0X"+QString("%1").arg(uart2can.paraInfo.codeNeed,4,16,QLatin1Char('0'))+"发生错误:"
                +((uart2can.paraErrorCode&PARA_READ_ERROR_CODE   )?"/无此功能码":"")
                +((uart2can.paraErrorCode&PARA_READ_ERROR_TYPE   )?"/类型错误":"")
                +((uart2can.paraErrorCode&PARA_READ_ERROR_INDEX  )?"/漏帧":"")
                +((uart2can.paraErrorCode&PARA_READ_ERROR_NOT_NEED)?"/编号不匹配":"")
                ;
        ui->labelParaInfo->setText(str);
        ui->pushButtonReadPara->setEnabled(true);
        break;
    ...
    case OTA_STA_WRITE_PARA_SUCCESS  :
        ...
        ui->tableWidgetPara->item(uart2can.paraInfo.posIndex+uart2can.paraInfo.posArea[uart2can.paraInfo.itemIndex].start,
                                  PARA_TAB_EDIT_COLUMN)->setForeground(QColor(Qt::black));
    ...
    case OTA_STA_WRITE_PARA_ERROR    :
        ...
        str = "写参数0X"+QString("%1").arg(uart2can.paraInfo.codeNeed,4,16,QLatin1Char('0'))+"发生错误:"
                +((uart2can.paraErrorCode&PARA_WR_ERROR_NO_CODE           )?"/无此参数":"")
                +((uart2can.paraErrorCode&PARA_WR_ERROR_FORBID_MODIFY     )?"/不可修改":"")
                +((uart2can.paraErrorCode&PARA_WR_ERROR_TYTE              )?"/类型不匹配":"")
                +((uart2can.paraErrorCode&PARA_WR_ERROR_LOSS_FRAME        )?"/漏帧":"")
                +((uart2can.paraErrorCode&PARA_WR_ERROR_FRAME             )?"/错帧":"")
                +((uart2can.paraErrorCode&PARA_WR_ERROR_LIMIT             )?"/设置数据超范围":"")
                +((uart2can.paraErrorCode&PARA_WR_ERROR_MODE_FORBID_MODIFY)?"/当前模式下不可修改":"")
                +((uart2can.paraErrorCode&PARA_WR_ERROR_MODE_FORBID_SAVE  )?"/当前模式下不可保存":"")
                +((uart2can.paraErrorCode&PARA_WR_ERROR_SAVE              )?"/保存失败":"")
                +((uart2can.paraErrorCode&PARA_WR_ERROR_NO_CMD            )?"/命令错误":"")
                +((uart2can.paraErrorCode&PARA_WR_ERROR_CMD_MATCH         )?"/命令不匹配":"")
                +((uart2can.paraErrorCode&PARA_WR_ERROR_CODE_MATCH        )?"/参数不匹配":"")
                ;
        ui->labelParaInfo->setText(str);
        ui->pushButtonWritePara->setEnabled(true);
        break;
    ...
    case OTA_STA_PARA_UPDATE_ERROR:
        ...
        str = "示波器配置发送错误:"+QString::number(uart2can.paraErrorCode,16)
                +((uart2can.paraErrorCode&PARA_UPDATE_ERROR_CMD         )?"/未知命令":"")
                +((uart2can.paraErrorCode&PARA_UPDATE_ERROR_FRESET      )?"/频率配置超出范围":"")
                +((uart2can.paraErrorCode&PARA_UPDATE_ERROR_CHSET_NOCODE)?"/未知变量":"")
                +((uart2can.paraErrorCode&PARA_UPDATE_ERROR_CHSET_FRE   )?"/数据超出频率可输出数量":"")
                +((uart2can.paraErrorCode&PARA_UPDATE_ERROR_CHSET_LOSE  )?"/错帧":"")
                +((uart2can.paraErrorCode&PARA_UPDATE_ERROR_START       )?"/启动错误":"")
                +((uart2can.paraErrorCode&PARA_UPDATE_ERROR_CHSET_TYPE  )?"/类型错误":"")
                ;
...
void MainWindow::on_pushButtonEchoSet_clicked()
{
    ...
    for(quint8 i=0;i<CHANNAL_SEL_NUM;i++)
    {
        ...
        //qDebug()<<"code"<<QString::number(uart2can.chSelCode[i],16)
        //      <<" type:"<<QString::number(uart2can.chTypeNeed[i],10);
...
void MainWindow::on_lineEditSetEchoFre_textChanged(const QString &arg1)
{
    ...
    {
        ui->label_selChInfo->setStyleSheet("color:red;");
        ui->label_selChInfo->setText("输出频率设置范围："
                                        +QString::number(uart2can.chEchoFreHzMin,10)+"~"
                                        +QString::number(uart2can.chEchoFreHzMax,10)+" HZ");
...
//void MainWindow::on_comboBoxSelMode_currentIndexChanged(const QString &arg1)
//{

//}
//void MainWindow::on_Burst_checkBox_stateChanged(int)
//{
//    if(ui->Burst_checkBox->isChecked())
//    {
//        uart2can.Burst_mode=1;

//    }
//    else
//    {
//        uart2can.Burst_mode=0;
//    }
//     uart2can.setCmd(STEP_SCOPE_SET_MODE);
