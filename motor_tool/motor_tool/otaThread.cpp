...
const qint32 otaThread::baudTab[BAUD_TAB_SIZE]={921600,9600,115200,1200,2400,4800,19200,38400,57600,230400,460800,1152000,};
...
otaThread::otaThread()//这应该是初始化函数，由专门的名称叫做啥忘记了。
{
    runStep = STEP_STOP;
...
void otaThread::getSerialData(QByteArray temp)
{
    ...
    while(pos >= 0)
    {
        rxPack = RxData.left(pos+2);
        RxData.remove(0,pos+2);
...
void otaThread::setCmd(enum canComStep mode)
{
    ...
    case STEP_WRITE_PARA_STR:
        ...
        else
        {
            ...
            case  WRITE_STR :
                              ...
                              break;
            case  WRITE_PARA: paraInfo.typeNeed = paraInfo.writePara[paraInfo.posIndex].type;
                              memcpy(p_buf,paraInfo.writePara[paraInfo.posIndex].valM,paraByteNumTab[paraInfo.typeNeed]);
                              break;
            default:paraInfo.writeErrorCode = PARA_WRITE_ERROR_FORBID_MODIFY;break;
            }
        }
    ...
    case STEP_SCOPE_SET_CHANNAL:
        ...
        pack.exId.mode = CANCOM_PARA_UPDATE;
        memcpy(&(pack.data[0]),&chSelCode[chSelIndex[chSelPos]],2);chSelPos++;
        memcpy(&(pack.data[2]),&chSelCode[chSelIndex[chSelPos]],2);chSelPos++;
        memcpy(&(pack.data[4]),&chSelCode[chSelIndex[chSelPos]],2);chSelPos++;
        memcpy(&(pack.data[6]),&chSelCode[chSelIndex[chSelPos]],2);chSelPos++;
        txdPack(&pack);
        break;
    ...
    case STEP_SCOPE_STOP:
      ...
      case STEP_SCOPE_SWEEP:   //扫频模式
        pack.exId.data = CAN_ID_DEBUG_UI | ((quint16)(Burst_mode&0X01)<<8);
        pack.exId.mode = CANCOM_MOTOR_SWEEP;
    }
...
void otaThread::analysisRxdDatas(QByteArray pack)
{
    ...
    if((pack.startsWith("AT"))&&(pack.size()==((quint8)pack[6]+9)))
    {
        ...
        */
        ...
        case CANCOM_ANNOUNCE_DEVID:rxdPack_ANNOUNCE_DEVID();break;
        case CANCOM_MOTOR_FEEDBACK:rxdPack_MOTOR_FEEDBACK();break;
        case CANCOM_CALI_ING      :rxdPack_CALI_ING();      break;
        case CANCOM_CALI_RST      :rxdPack_CALI_RST();      break;
        case CANCOM_HALLCALI_ING  :rxdPack_HALLCALI_ING();  break;
        case CANCOM_HALLCALI_RST  :rxdPack_HALLCALI_RST();  break;
        case CANCOM_MOTOR_CALI    :rxdPack_MOTOR_CALI();    break;
        case CANCOM_PARA_STR_INFO :rxdPack_PARA_STR_INFO(); break;
        case CANCOM_PARA_READ     :rxdPack_PARA_READ();     break;
        case CANCOM_PARA_WRITE    :rxdPack_PARA_Write();    break;
        case CANCOM_PARA_UPDATE   :rxdPack_PARA_UPDATE();   break;
        case CANCOM_FAULT_WARN    :rxdPack_FAULT_WARN();    break;
        case CANCOM_MOTOR_SWEEP   :rxdPack_MOTOR_SWEEP();   break;
...
//           }
//        else if ((isAFrame == true)&&(rxFrame.exId.mode == CANCOM_MOTOR_SWEEP)&&(mtStatus.mode==MT_MODE_MOTOR_17))
//            hasAcked = true;
        break;
        ...
        case STEP_GET_DEV_INFO:
           ...
           #define MAIN_SCAN_MAX  (128/SUB_SCAN_MAX)
            ...
            else
            {
                ...
                pack.exId.data = CAN_ID_DEBUG_UI;
                pack.len = 0;
                pack.exId.mode = CANCOM_ANNOUNCE_DEVID;
                txdPack(&pack);
                msleep(4);
            }
            break;
...
//扫频函数
void otaThread::rxdPack_MOTOR_SWEEP()//扫频
{
        ...
        // 发出信号通知界面更新
        otaStatus = OTA_STA_HALL_CALI_SWEEP;
        emit sendState();
...
void otaThread::rxdPack_PARA_STR_INFO()
{
    ...
    switch (item) {
    case PARA_NAME0: paraCode = 0;memset(&p_buf[0],0,18);memcpy(&p_buf[0],&rxFrame.data[2],6);break;
    case PARA_NAME1: memcpy(&p_buf[6],&rxFrame.data[2],6);
                    if(paraItemBak != PARA_NAME0)
                        paraCode |= PARA_ERROR_ITEM;
                    break;
    case PARA_NAME2: memcpy(&p_buf[12],&rxFrame.data[2],6);p_buf[STR_NAME_LEN]=0;
                    if(paraItemBak != PARA_NAME1)
                        paraCode |= PARA_ERROR_ITEM;
                    else if(point < paraNumTab[type])
                    {
                        switch (type) {
                        case WRITE_STR:  paraInfo.writeStr[point].name = QString(QLatin1String(p_buf));break;
                        case READ_STR:   paraInfo.readStr[point].name  = QString(QLatin1String(p_buf));break;
                        case WRITE_PARA: paraInfo.writePara[point].name= QString(QLatin1String(p_buf));break;
                        case READ_PARA:  paraInfo.readPara[point].name = QString(QLatin1String(p_buf));break;
                        default:paraCode |= PARA_ERROR_ITEM_MATCH;break;
                        }
                    }
                    break;
    case PARA_ATTR:  paraCode = 0;
                    if(point < paraNumTab[type])
                    {
                        switch (type) {
                        case WRITE_PARA: paraInfo.writePara[point].type= (enum paraType)(rxFrame.data[7]&0X1F);
                                         ...
                                         break;
                        case READ_PARA:  paraInfo.readPara[point].type= (enum paraType)(rxFrame.data[7]&0X1F);
                                         break;
                            default:paraCode |= PARA_ERROR_ITEM_MATCH;break;
                        }
                    }
                    break;
    case PARA_VAL0:  paraCode = 0;
                     if(point < paraNumTab[type])
                     {
                         switch (type) {
                         case WRITE_PARA: paraInfo.writePara[point].type= (enum paraType)rxFrame.data[3];
                                          memcpy(&(paraInfo.writePara[point].valR[0]),&rxFrame.data[4],4);
                                          break;
                         case READ_PARA:  paraInfo.readPara[point].type= (enum paraType)rxFrame.data[3];
    ...
    case PARA_VAL1:
                    ...
                    break;
    case PARA_STR0:  paraCode = 0;memset(&p_buf[0],0,18);memcpy(&p_buf[0],&rxFrame.data[2],6);break;
    case PARA_STR1:  memcpy(&p_buf[6],&rxFrame.data[2],6);
                    if(paraItemBak != PARA_STR0)
                        paraCode |= PARA_ERROR_ITEM;
                    break;
    case PARA_STR2:  memcpy(&p_buf[12],&rxFrame.data[2],6);p_buf[STR_NAME_LEN]=0;
                    if(paraItemBak != PARA_STR1)
                        paraCode |= PARA_ERROR_ITEM;
                    else if(point < paraNumTab[type])
                    {
                        switch (type) {
                        case WRITE_STR:  paraInfo.writeStr[point].str = QString(QLatin1String(p_buf));break;
                        case READ_STR:   paraInfo.readStr[point].str  = QString(QLatin1String(p_buf));break;
                        default:paraCode |= PARA_ERROR_ITEM_MATCH;break;
                        }
                    }
                    break;
    case PARA_DONE:paraCode = 0;
                    ...
                    paraItemBak  = item;
                    return;
    default: paraCode |= PARA_ERROR_NO_TYPE;break;
    }
...
void otaThread::rxdPack_PARA_READ()
{
    ...
    else
    {
        ...
        if(indexBak >= indexMax)
        {
            ...
            switch(paraInfo.itemIndex){
            case WRITE_STR:paraInfo.writeStr[paraInfo.posIndex].str = QString(QLatin1String(p_buf));break;
            case READ_STR : paraInfo.readStr[paraInfo.posIndex].str = QString(QLatin1String(p_buf));break;
            case WRITE_PARA:memcpy(&(paraInfo.writePara[paraInfo.posIndex].valR[0]),p_buf,paraByteNumTab[type]);break;
            case READ_PARA: memcpy(&( paraInfo.readPara[paraInfo.posIndex].valR[0]),p_buf,paraByteNumTab[type]);break;
            default:errorCode |= PARA_READ_ERROR_CODE;break;
            }
        }
        else
            return;
...
void otaThread::rxdPack_PARA_UPDATE()
{
    ...
    case 1:
        ...
        case 0:
                ...
                break;
         //下面是报错可以暂时不理会
        case 1:paraErrorCode |= PARA_UPDATE_ERROR_CHSET_NOCODE;otaStatus = OTA_STA_PARA_UPDATE_ERROR;break;
        case 2:paraErrorCode |= PARA_UPDATE_ERROR_CHSET_FRE;otaStatus = OTA_STA_PARA_UPDATE_ERROR;break;
        case 3:
        default:paraErrorCode |= PARA_UPDATE_ERROR_CHSET_LOSE;otaStatus = OTA_STA_PARA_UPDATE_ERROR;break;
        }
        break;
    case 2:scopeIngFlag = true;//
        if(ack)
        {
        ...
        break;
    case 3:scopeIngFlag = false;otaStatus = OTA_STA_PARA_UPDATE_SUCCESS_END;break;
    default:paraErrorCode|=PARA_UPDATE_ERROR_CMD;otaStatus = OTA_STA_PARA_UPDATE_ERROR;break;
    }
    emit sendState();
}
...
void otaThread::scope_dataProcess()//
{
    ...
    if(Burst_mode)
    {
        if(tick > chTickBak)
            ...
            chTimeStamp += 50*((tick - chTickBak)/1000000.0f);//转换成以秒为单位
    ...
    if(tick > chTickBak)
    {
        chTimeStamp += (tick - chTickBak)*chTickUs/10000000.0f;//转换成以微秒为单位
    }
    else
    {
        chTimeStamp += (65536+tick - chTickBak)*chTickUs/10000000.0f;//转换成以微秒为单位
    }
    chTickBak = tick;//更新时间
    }
    ...
    for(quint8 i =0;i<chSelNum;i++)
    {
        ...
        switch(type)
        {
            case ParaType_U8     :quint8  tempU8 ; memcpy(&tempU8 ,&chRxdByte[pos],paraByteNumTab[type]);val = tempU8 *1.0f; pStr = QString::asprintf("%4d",tempU8 );break;
            case ParaType_S8     :qint8   tempS8 ; memcpy(&tempS8 ,&chRxdByte[pos],paraByteNumTab[type]);val = tempS8 *1.0f; pStr = QString::asprintf("%5d",tempS8 );break;
            case ParaType_U16    :quint16 tempU16; memcpy(&tempU16,&chRxdByte[pos],paraByteNumTab[type]);val = tempU16*1.0f; pStr = QString::asprintf("%6d",tempU16);break;
            case ParaType_S16    :qint16  tempS16; memcpy(&tempS16,&chRxdByte[pos],paraByteNumTab[type]);val = tempS16*1.0f; pStr = QString::asprintf("%7d",tempS16);break;
            case ParaType_U32    :quint32 tempU32; memcpy(&tempU32,&chRxdByte[pos],paraByteNumTab[type]);val = tempU32*1.0f; pStr = QString::asprintf("%10d",tempU32);break;
            case ParaType_S32    :qint32  tempS32; memcpy(&tempS32,&chRxdByte[pos],paraByteNumTab[type]);val = tempS32*1.0f; pStr = QString::asprintf("%11d",tempS32);break;
            case ParaType_FLOAT  :float   tempSf ; memcpy(&tempSf ,&chRxdByte[pos],paraByteNumTab[type]);val = tempSf      ; pStr = QString::asprintf("%12.6f",tempSf);break;
            case ParaType_U64    :quint64 tempU64; memcpy(&tempU64,&chRxdByte[pos],paraByteNumTab[type]);val = tempU64*1.0f; pStr = QString::asprintf("%15l",tempU64);break;
            case ParaType_S64    :qint64  tempS64; memcpy(&tempS64,&chRxdByte[pos],paraByteNumTab[type]);val = tempS64*1.0f; pStr = QString::asprintf("%16l",tempS64);break;
            case ParaType_DOUBLE :                 memcpy(&val    ,&chRxdByte[pos],paraByteNumTab[type]);                    pStr = QString::asprintf("%16.8f",val);break;
            default: val = 0.0f;pStr="null";break;
        }
    ...
    if(isWaveFileOpen)
    {
        str = QString::asprintf("%14.6f",chTimeStamp)+str+"\r\ ";
